...
|---|---|---|---|
| `[FONTE]` | página aberta, com URL exata, data e frase literal | sim | sim |
...
2. **Duas datas por dado**: quando foi publicado e quando você abriu.
   Fonte sem data visível entra como `publicado em: não declarada`, e o
   texto não pode dizer "hoje" nem nomear o ano sobre ela.
...
6. **Apure exato, publique faixa.** Preço, consumo e prazo entram exatos
   aqui e saem como faixa no texto, com mês e ano. Faixa só alarga.
...
- [ ] Toda pergunta do contrato tem ao menos um dado `[FONTE]` ou um
      `[SEM DADO]` explícito.
- [ ] Todo `[FONTE]` tem URL direta, duas datas, classe e frase literal.
...
- [ ] "O que o leitor diz" tem ao menos cinco frases literais.
