...
SEO on-page (o que a etapa 6 confere):
  título < 60 e query nos primeiros 40 caracteres; description entre 140 e
  160; 4 ou mais H2 antes de "Fontes"; FAQ com 3 a 8 perguntas; query nas
  primeiras 100 palavras (abertura incluída); ao menos um link externo no
  corpo (não só na lista de fontes); figuras.
"""
import re, sys, unicodedata

# A faixa da description era 150 a 160. Medida contra um acervo de 269 artigos
# publicados, ela reprovava 28% deles por description de 140 a 149, tamanho que
# o buscador não trunca e que não é defeito. Abaixo de 140 sobra espaço no
# resultado; acima de 160 corta no meio. Com 140 a 160 a reprovação cai para
# 5%, e o que sobra são casos reais.
DESC_MIN, DESC_MAX = 140, 160
# O teto da FAQ existe para pegar a seção que virou um segundo artigo. Sete
# perguntas bem feitas não são isso, e cortar a sétima é perder resposta que a
# busca faz.
FAQ_MIN, FAQ_MAX = 3, 8
...
def main():
    ...
    print(f"  {ok(DESC_MIN <= len(desc) <= DESC_MAX)}  description: {len(desc)} caracteres  (régua: {DESC_MIN} a {DESC_MAX})")
    print(f"  {ok(len(h2) >= 4)}  H2 antes de Fontes: {len(h2)}  (régua: >= 4, senão o sumário não nasce)")
    print(f"  {ok(FAQ_MIN <= faq <= FAQ_MAX)}  perguntas na FAQ: {faq}  (régua: {FAQ_MIN} a {FAQ_MAX})")
    if termos:
